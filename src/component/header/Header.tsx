...
const Header: FC = () => {
    ...
    const getSearchTypes = () =>
        SEARCH_TYPES.map(element => {
            return (<S.SearchElement onClick={() => changeType(element)}>{element}</S.SearchElement>)
        });
    ...
    return (
        <S.HeaderBody>
            ...
            <S.SearchBox open={open}>
                ...
                <S.SearchInput placeholder="찾으실 검색어를 입력해주세요" onKeyPress={searchKeyword}></S.SearchInput>
                <S.SearchButtonIcon src={SearchButton}></S.SearchButtonIcon>
            </S.SearchBox>
            <S.HeaderButtonBox>
                <S.HeaderButton>책 등록</S.HeaderButton>
